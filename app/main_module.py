...
def enter_address():
    ...
    address = request.args.get('address')  # Get address
    client = Client(api_key)  # Connect to yandex-geocoder
    coordinates = client.transform_to_coordinates(address)  # Transform address to coordinates
...
def enter_coordinates():
    ...
    coordinates = Decimal(lat), Decimal(lon)  # Transform coordinates to a tuple of Decimals.
    if mkad.contains(Point(coordinates)):  # Check if coordinates are inside MKAD
